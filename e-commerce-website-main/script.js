...
(function () {
    document.addEventListener('DOMContentLoaded', () => {
      // Normalize image paths to images/ folder if not already set
      try {
        document.querySelectorAll('img').forEach(img => {
          const srcAttr = img.getAttribute('src');
          if (srcAttr && !srcAttr.includes('/') && !srcAttr.startsWith('data:')) {
            img.src = `images/${srcAttr}`;
          }
        });
        console.log('Image paths normalized to images/ folder');
      } catch (e) {
        console.error('Error normalizing image paths:', e);
      }
